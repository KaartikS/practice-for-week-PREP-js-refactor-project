...
function countConsonants(word) {
    const lettersSymbolsToExclude = ["a", "e", "i", "o", "u", " "];
    let count = 0;
    for (let i = 0; i < word.length; i++) {
        if (!(lettersSymbolsToExclude.includes(word[i]))) {
            count++;
        }
    }
    return count;
...
function alternatingLetters(str) {
    ...
    // lowercase the while string and move into array
    moddedStringArray = str.toLowerCase().split("");

    for (i = 0; i < moddedStringArray.length; i++) {
        // uppercase every odd position
        if (!(i % 2 == 0)) {
            moddedStringArray[i] = moddedStringArray[i].toUpperCase();
        }
    }
    return moddedStringArray.join('');
}
